...
RUN pip3 install -U setuptools parsy docopt pytest-tap --break-system-packages && git clone https://github.com/joesuf4/jinjalint /tmp/jinjalint && (cd /tmp/jinjalint && python3 setup.py install && sed -i -e "s/findall[\\(]'/findall(r'/g" -e "s/split[\\(]'/split(r'/g" /usr/local/lib/python3.*/dist-packages/docopt.py)
...
RUN cpan -f install sealed Algorithm::Diff LCS::BV LCS::XS B::Lint IO::Select URI Term::ReadKey YAML::XS HTML::Escape Cpanel::JSON::XS URI::Escape Digest::SHA1 FreezeThaw Dotiac::DTL::Addon::markup Time::timegm PDL::IO::CSV
#RUN cpan -f install Lucy
